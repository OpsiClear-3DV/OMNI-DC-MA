...
For the bicycle benchmark, the unmodified OMNI-DC+MA baseline at commit `1e1987b` measured 1.337 s for one image when run with the same higher-certainty sparse depth input, resized to the same 512-long-side image size, and padded to `352x512`. The retained 512-preview representative modes run 16 images at the same padded shape; the speed gain compares original single-image throughput against preview batch throughput at the same image size.
...
| low, `metric_generic16` | 16 | `352x512` | ~0.891 s | ~0.0557 s | ~24.0x | 0.009102 m | 0.022091 m |
| mid, `hybrid_calibrated16` | 16 | `352x512` | ~0.890 s | ~0.0556 s | ~24.0x | 0.008479 m | 0.025354 m |
| high, `metric_highspan16` | 16 | `352x512` | ~0.886 s | ~0.0554 s | ~24.1x | 0.011834 m | 0.025463 m |

The error columns are final-output representative approximation error against the 512-preview all-frame teacher, not ground-truth depth error.
