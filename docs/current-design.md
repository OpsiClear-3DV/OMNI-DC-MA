...
Shared COLMAP code lives in `src/colmap_utils/`. `filters.py` and `sparse_depth.py` hold the reusable sparse-anchor filtering path. `editing.py` holds point-cloud editing primitives that keep `points3D` and per-image `point3D_ids` consistent when points or observations are added or removed. `densify.py` adds depth-guided single-view points in underfilled image-grid cells, using dense metric depth maps as the source of camera-z depth. The existing sparse-depth CLI filters generated anchors; editing and densification helpers are the base for future tools that rewrite COLMAP models.
...
The MA prior can extrapolate unconstrained far-field depth beyond the sparse SfM anchors. The sky/far mask path is off by default and runs only when `--sky_mask` or `-sky_mask` is passed. `apply_anchor_cap` caps predictions above `far_depth_factor * max(valid sparse depth)` and writes those pixels as `0`, preserving the same invalid-depth sentinel used by sparse inputs. The older CLI name, `anchor_cap_factor`, remains an alias. The prior sky/far-field mask is computed in MA metric-depth space using COLMAP focal scaling when available and applied to the saved completed depth by default when the feature is enabled.

Use `--no_apply_sky_mask` to compute/export masks without zeroing the saved completed depth.

When `--sky_mask` is enabled and `--save_colmap_mask` or `--demo_outputs ... colmap_mask` is requested, `demo.py` also writes a COLMAP `mask_path`-compatible PNG beside the image tree, e.g. `scene/images/frame.jpg` -> `scene/masks/frame.jpg.png`. These masks use COLMAP semantics: white pixels are kept, black pixels are ignored. Preview-resolution masks are resized back to the original RGB image size before saving.
