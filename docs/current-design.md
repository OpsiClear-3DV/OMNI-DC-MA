...
The model expects an RGB image plus a sparse metric-depth `.npy` with the same image frame. Valid depth values are in meters; invalid pixels are `0`. Sparse maps generated by `tools/generate_colmap_sparse_depth.py` use higher-certainty COLMAP points by default: track length at least 3 and reprojection error at most 2 px.
