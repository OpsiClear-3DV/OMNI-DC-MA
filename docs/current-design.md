...
Shared COLMAP code lives in `src/colmap_utils/`. `filters.py` and `sparse_depth.py` hold the reusable sparse-anchor filtering path. `editing.py` holds point-cloud editing primitives that keep `points3D` and per-image `point3D_ids` consistent when points or observations are added or removed. The existing sparse-depth CLI filters generated anchors; editing helpers are the base for future tools that rewrite COLMAP models.
