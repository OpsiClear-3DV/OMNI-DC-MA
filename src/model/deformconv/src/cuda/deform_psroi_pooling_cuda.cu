...
// THC headers removed in torch 2.x; AT_CUDA_CHECK / c10::cuda is the replacement.
// #include <THC/THC.h>
                                  ...
                                  const float trans_std)
{
  ...
  dim3 grid(static_cast<unsigned int>(std::min(static_cast<int64_t>((out_size + 511L) / 512L), static_cast<int64_t>(4096L))));
  dim3 block(512);
