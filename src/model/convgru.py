...
class BasicUpdateBlock(nn.Module):
    def __init__(self, args, resolution=1, hidden_dim=64, input_dim=64, mask_r=8, conf_min=0.01):
        ...
        self.resolution = resolution
        self.register_buffer("_uniform_grad_weights",
                             torch.ones(resolution, dtype=torch.float32) / float(resolution))
        area_weights = torch.tensor([float(4 ** i) for i in range(resolution)], dtype=torch.float32)
        self.register_buffer("_uniform_area_grad_weights", area_weights / area_weights.sum())
        self.register_buffer("_uniform_input_weights",
                             torch.tensor([1.0] + [0.0] * (resolution - 1), dtype=torch.float32))
    ...
    def forward(self, net, inp, depth, depth_grad, K):
        ...
        for i in range(self.resolution):
            r = 2 ** i
            delta_depth_grad[:, i*2:(i+1)*2, :H//r, :W//r].copy_(delta_depth_grad_raw[i])
        ...
        if self.args.multi_resolution_learnable_gradients_weights == "learnable":
            ...
            weights_depth_grad = self._uniform_grad_weights.to(dtype=net.dtype)
            ...
            weights_depth_grad = self._uniform_area_grad_weights.to(dtype=net.dtype)
            weights_depth_grad = weights_depth_grad.reshape(1, self.resolution, 1, 1).repeat(B, 2, H, W)
        else:
        ...
        if self.args.multi_resolution_learnable_input_weights:
            ...
            weights_input = self._uniform_input_weights.to(dtype=net.dtype)
