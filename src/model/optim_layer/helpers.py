...
def multires_sparse_depth_downsample(batched_sparse_depth, batched_valid_sparse_mask, resolution):
    ...
    batched_sparse_depth = torch.where(
        batched_valid_sparse_mask == 0.0,
        torch.zeros_like(batched_sparse_depth),
        batched_sparse_depth,
    )
    ...
    for i in range(resolution):
        ...
        valid = valid_mask_downsampled > 0.0
        sparse_depth_downsampled = torch.where(
            valid,
            sparse_depth_downsampled / valid_mask_downsampled.clamp(min=1e-12),
            torch.zeros_like(sparse_depth_downsampled),
        )
        valid_mask_downsampled = valid.to(valid_mask_downsampled.dtype)
