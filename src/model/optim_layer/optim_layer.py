...
# Jacobi preconditioner toggle (set from config: --cg_no_precond disables).
# Default ON: it's numerically safe (positive SPD M can't change the
# converged solution, only the iteration count) and is the whole point.
_USE_JACOBI = True


def set_jacobi(enabled: bool) -> None:
    global _USE_JACOBI
    _USE_JACOBI = bool(enabled)
...
class DepthGradOptimLayer(torch.autograd.Function):
    ...
    def forward(ctx, batched_image_gradients, batched_sparse_depth,
                ...
                b_init=None, lamda=0.1, rtol=1e-5, max_iter=1000, check_interval=1, fixed_iters=False):
        ...
        ctx.fixed_iters = bool(fixed_iters)
        ...
        # Sparse-depth terms currently use only the base resolution
        # (the loops below intentionally run range(1)). Avoid building unused
        # pooled sparse-depth/mask levels.
        multires_batched_sparse_depth = [batched_sparse_depth]
        multires_batched_valid_sparse_mask = [batched_valid_sparse_mask]
        ...
        # Jacobi (diagonal) preconditioner. The system AᵀWA + diag(λ·c·mask)
        # is a confidence-weighted Poisson/data system — κ∼O(N) ill-conditioned,
        # which is exactly why unpreconditioned CG needs ~833 iters here.
        # Exact diagonal via stencil-coloring probing of the SAME operator:
        # the AᵀWA term is a radius-1 5-point Laplacian, so probe vectors that
        # are 1 on a mod-3 sublattice (9 colors) isolate the diagonal exactly
        # (nearest same-color pixels are 3 apart > the stencil radius). 9
        # matvecs, layout-agnostic, provably exact, negligible vs ~833 iters.
        # A positive SPD M leaves the converged solution unchanged (only the
        # iteration path/count), so this is numerically safe by construction.
        M_bmm = None
        if _USE_JACOBI:
            with torch.no_grad():
                diag = torch.zeros(batch_size, H * W, 1, device=device, dtype=dtype)
                ii = torch.arange(H, device=device)
                jj = torch.arange(W, device=device)
                cmask = (ii[:, None] % 3) * 3 + (jj[None, :] % 3)  # H×W, colors 0..8
                cmask = cmask.reshape(-1)
                for c in range(9):
                    e = (cmask == c).to(dtype).reshape(1, H * W, 1).expand(batch_size, -1, -1)
                    diag = torch.where(e.bool(), batched_RTRp(e), diag)
                inv_diag = 1.0 / diag.clamp(min=1e-6)

            def M_bmm(v):
                return v * inv_diag

        x, info = cg_batch(
            batched_RTRp, b, M_bmm=M_bmm, X0=x_init, rtol=rtol, maxiter=max_iter,
            check_interval=check_interval, fixed_iters=ctx.fixed_iters, verbose=False
        )
    ...
    def backward(ctx, gradx, grad_b_init):
        ...
        gradb, info = cg_batch(
            batched_RTRp, gradx, X0=grad_b_init, rtol=ctx.rtol, maxiter=ctx.max_iter,
            check_interval=ctx.check_interval, fixed_iters=ctx.fixed_iters, verbose=False
        )
        ...
        return grad_batched_image_gradients, None, None, grad_batched_confidence, grad_batched_input_confidence, \
                ...
                grad_lamda, None, None, None, None
